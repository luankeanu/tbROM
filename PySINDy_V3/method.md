...
## 2026-04-13: Split-model V3 baseline

### Objective of this increment
V3 changes the modelling strategy from one shared two-state PySINDy model to
two separate one-output PySINDy models. The intent is to let `cl` and `cd`
be learned independently while keeping the same control inputs and the same
overall workflow stages.

### Canonical V3 signal definition
The V3 control inputs are:
- `pitch`
- `pitch_rate`

The V3 prediction targets are:
- `cl`
- `cd`

This means each fitted model now solves a one-output problem:
- one model predicts the time evolution of `cl`
- one model predicts the time evolution of `cd`

### Structural change relative to V2
The split applies to the training, run, and tuning stages.

In practice:
- `assets/model.py` builds one target-specific PySINDy model at a time
- `assets/fit.py` fits two models and stores them together as one bundled artifact
- `assets/run.py` simulates the `cl` and `cd` models separately and then joins the results
- `assets/analyse.py` can continue to work from the joined prediction outputs

### Why this version is useful
The split-model design removes direct coupling between `cl` and `cd` inside the
identified equations. This is worth testing because the single shared V2 model
showed damping and stability issues on confirmation runs, and a separated
formulation may be easier to tune and interpret.
