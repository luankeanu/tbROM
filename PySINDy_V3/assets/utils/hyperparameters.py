...
file, matching the explicit coursework style. In V3, every candidate setting
is applied to two separate one-output models, one for `cl` and one for `cd`.
"""
...
"""
Function: _fit_candidate_models.
Fits the two separate V3 PySINDy target models for one candidate setting
dictionary using the same multi-trajectory conventions as the normal fit stage.
"""
...
def _fit_candidate_models(
    ...
    for target_column in model.get_target_columns():
        ...
        x_train, u_train, t_train, _ = fit._build_training_trajectories(
        ...
        fitted_models[target_column] = fitted_model
...
def _append_equation_history(
    ...
    for target_column in model.get_target_columns():
        ...
        equations = fit._extract_equations(
            best_models[target_column],
            target_column=target_column,
        )
        section_lines.append(f"{target_column.upper()} model:")
        section_lines.append(f"Library: {model_summary['library_type']}")
        section_lines.append(f"Optimizer: {model_summary['optimizer_type']}")
        section_lines.append("Equations:")
...
Runs the explicit brute-force V3 tuning workflow using the selected validation
...
def run_hyperparameter_search() -> HyperparameterSearchOutput:
        ...
        for alpha in ALPHA_VALUES:
            for polynomial_degree in POLYNOMIAL_DEGREE_VALUES:
                ...
                for fold_index, holdout_name in enumerate(validation_case_names, start=1):
                    ...
                    try:
                        fitted_models = _fit_candidate_models(
    ...
    best_models = _fit_candidate_models(
