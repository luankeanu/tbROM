...
discard the first 500 numeric rows and compute the canonical `pitch(t)` and
...
"""
Function: compute_pitch_rate_degrees.
Computes the first derivative of the pitch history with respect to `flow_time`.
The result remains in degree-based units because `pitch` itself is stored in
degrees throughout the current V2 workflow.
"""


def compute_pitch_rate_degrees(
    prepared_table: pd.DataFrame,
    pitch_values: np.ndarray,
) -> np.ndarray:
    time_values = prepared_table["flow_time"].to_numpy(dtype=float)
    if len(time_values) <= 1:
        return np.zeros(len(time_values), dtype=float)
    edge_order = 2 if len(time_values) >= 3 else 1
    return np.gradient(pitch_values, time_values, edge_order=edge_order)


"""
Function: build_prepared_case_table.
Creates the table that later scripts will actually consume. This is where the
case metadata is attached to every retained row and where `pitch(t)` and
`pitch_rate(t)` are added.
"""
...
def build_prepared_case_table(
    ...
    pitch_values = compute_pitch_degrees(prepared_table)
    prepared_table["pitch"] = pitch_values
    prepared_table["pitch_rate"] = compute_pitch_rate_degrees(
        ...
        rows.append(
            {
                ...
                "pitch_rate_min": float(table["pitch_rate"].min()),
                "pitch_rate_max": float(table["pitch_rate"].max()),
