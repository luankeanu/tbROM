...
def train_knn(X_train, y_train, X_test, y_test, n_neighbors=3, weights='distance', p=2):
    """Trains a KNN Regressor model with specified hyperparameters."""
    ...
    r2_roughness_knn_test = r2_score(y_test['roughness'], y_pred_knn_test[:, 0])
    mse_roughness_knn_test = mean_squared_error(y_test['roughness'], y_pred_knn_test[:, 0])
    rmse_roughness_knn_test = np.sqrt(mse_roughness_knn_test)
    mae_roughness_knn_test = mean_absolute_error(y_test['roughness'], y_pred_knn_test[:, 0])
    roughness_pe_avg_knn_test = calculate_average_percentage_error(y_test['roughness'], y_pred_knn_test[:, 0])

    r2_tensile_strength_knn_test = r2_score(y_test['tension_strength'], y_pred_knn_test[:, 1])
    mse_tensile_strength_knn_test = mean_squared_error(y_test['tension_strength'], y_pred_knn_test[:, 1])
    rmse_tensile_strength_knn_test = np.sqrt(mse_tensile_strength_knn_test)
    mae_tensile_strength_knn_test = mean_absolute_error(y_test['tension_strength'], y_pred_knn_test[:, 1])
    tensile_strength_pe_avg_knn_test = calculate_average_percentage_error(y_test['tension_strength'], y_pred_knn_test[:, 1])

    r2_elongation_knn_test = r2_score(y_test['elongation'], y_pred_knn_test[:, 2])
    mse_elongation_knn_test = mean_squared_error(y_test['elongation'], y_pred_knn_test[:, 2])
    rmse_elongation_knn_test = np.sqrt(mse_elongation_knn_test)
    mae_elongation_knn_test = mean_absolute_error(y_test['elongation'], y_pred_knn_test[:, 2])
    elongation_pe_avg_knn_test = calculate_average_percentage_error(y_test['elongation'], y_pred_knn_test[:, 2])

    #R2, MSE, RMSE, MAE, and %E avg for each target variable (Train set)
    r2_roughness_knn_train = r2_score(y_train['roughness'], y_pred_knn_train[:, 0])
    mse_roughness_knn_train = mean_squared_error(y_train['roughness'], y_pred_knn_train[:, 0])
    rmse_roughness_knn_train = np.sqrt(mse_roughness_knn_train)
    mae_roughness_knn_train = mean_absolute_error(y_train['roughness'], y_pred_knn_train[:, 0])
    roughness_pe_avg_knn_train = calculate_average_percentage_error(y_train['roughness'], y_pred_knn_train[:, 0])

    r2_tensile_strength_knn_train = r2_score(y_train['tension_strength'], y_pred_knn_train[:, 1])
    mse_tensile_strength_knn_train = mean_squared_error(y_train['tension_strength'], y_pred_knn_train[:, 1])
    rmse_tensile_strength_knn_train = np.sqrt(mse_tensile_strength_knn_train)
    mae_tensile_strength_knn_train = mean_absolute_error(y_train['tension_strength'], y_pred_knn_train[:, 1])
    tensile_strength_pe_avg_knn_train = calculate_average_percentage_error(y_train['tension_strength'], y_pred_knn_train[:, 1])

    r2_elongation_knn_train = r2_score(y_train['elongation'], y_pred_knn_train[:, 2])
    mse_elongation_knn_train = mean_squared_error(y_train['elongation'], y_pred_knn_train[:, 2])
    rmse_elongation_knn_train = np.sqrt(mse_elongation_knn_train)
    mae_elongation_knn_train = mean_absolute_error(y_train['elongation'], y_pred_knn_train[:, 2])
    elongation_pe_avg_knn_train = calculate_average_percentage_error(y_train['elongation'], y_pred_knn_train[:, 2])

    metrics = {
        "Module name": "KNN Regressor (Selected Features)",
        "R2 (Test)": r2_knn_test,
